...
class FRSCommands:
    ...
    def _018(msg: can.Message): #24
        ...
        translatedMessage = Message(msg.timestamp, 'Steering Angle One', round(int.from_bytes([msg[0], msg[1]], 'little') * 0.1, 2))
        #steering_angle_one = round(int.from_bytes([msg[0], msg[1]], 'little') * 0.1, 2)
        return translatedMessage

    def _0D0(msg: can.Message): #208
        steering_angle_two = Message(msg.timestamp, 'Steering Angle Two', round(int.from_bytes([msg[0], msg[1]], 'little') * 0.1, 2))
        yaw_rate = Message(msg.timestamp,'',round(int.from_bytes([msg[2], msg[3]], 'little') * -0.286478897, 2))

        lateral_accel = Message(msg.timestamp,'',round(msg[6] * 0.2, 2))
        long_accel = Message(msg.timestamp,'',round(msg[7] * -0.1, 2))
        return steering_angle_two, yaw_rate, lateral_accel, long_accel


    def _0D1(msg: can.Message): #209
        speed = Message(msg.timestamp,'Speed',round(int.from_bytes([msg[0],msg[1]],'little'), 2))
        brake_pressure = Message(msg.timestamp,'Brake Pressure',min(msg[2] / 0.7, 100))


    def _0D2(msg: can.Message): #210
        # unknown
        _210 = Message(msg.timestamp,'210',msg.data)
    def _0D3(msg: can.Message): #211
        # unknown
        _211 = Message(msg.timestamp,'211',msg.data)
    def _0D4(msg: can.Message): #212
        wheel_speed_FL = Message(msg.timestamp,'Wheel Speed FL',int.from_bytes([msg[0], msg[1]], 'little')) #0,1
        wheel_speed_FR = Message(msg.timestamp,'Wheel Speed FR',int.from_bytes([msg[2], msg[3]], 'little')) #2,3
        wheel_speed_RL = Message(msg.timestamp,'Wheel Speed RL',int.from_bytes([msg[4], msg[5]], 'little')) #4,5
        wheel_speed_RR = Message(msg.timestamp,'Wheel Speed RR',int.from_bytes([msg[6], msg[7]], 'little')) #6,7
        
    def _140(msg: can.Message): #dec 320
        accelerator_position_percent = Message(msg.timestamp,'Accelerator Position Percent',msg[7] / 2.55)
        throttle_position = Message(msg.timestamp,'Throttle Position',msg[6] / 2.55)
    ...
    def _141(msg: can.Message): #321
        _321 = Message(msg.timestamp,'321',msg.data)
        
    def _142(msg: can.Message): #322
        _322 = Message(msg.timestamp,'322',msg.data)
        # unknown
        
    def _144(msg: can.Message): #324
        _324 = Message(msg.timestamp,'324',msg.data)
    def _152(msg: can.Message): #338
        _338 = Message(msg.timestamp,'338',msg.data)
    def _156(msg:can.Message): #342
        _342 = Message(msg.timestamp,'342',msg.data)
    def _282(msg: can.Message): #642
        _642 = Message(msg.timestamp,'642',msg.data)
    def _360(msg: can.Message): #864
        ...
        # cruise set
        # cruise speed
        _864 = Message(msg.timestamp,'864',msg.data)
        
    def _361(msg: can.Message): #865
        _865 = Message(msg.timestamp,'865',msg.data)
        # gear
        
    def _370(msg: can.Message): #880
        _880 = Message(msg.timestamp,'880',msg.data)
        
    def _372(msg: can.Message): #882
        _882 = Message(msg.timestamp,'882',msg.data)
        
    def _374(msg: can.Message): #884
        _884 = Message(msg.timestamp,'884',msg.data)
        
    def _375(msg: can.Message): #885
        _885 = Message(msg.timestamp,'885',msg.data)
        
    def _440(msg: can.Message): #1088
        _1088 = Message(msg.timestamp,'1088',msg.data)
        
    def _442(msg: can.Message): #1090
        _1090 = Message(msg.timestamp,'1090',msg.data)
        
    def _63B(msg: can.Message): #1595
        _1595 = Message(msg.timestamp,'1595',msg.data)
        
    def _4C1(msg: can.Message): #1217
        _1217 = Message(msg.timestamp,'1217',msg.data)
        
    def _4C3(msg: can.Message): #1219
        _1219 = Message(msg.timestamp,'1219',msg.data)
        
    def _4C6(msg: can.Message): #1222
        _1222 = Message(msg.timestamp,'1222',msg.data)
        
    def _4C8(msg: can.Message): #1224
        _1224 = Message(msg.timestamp,'1224',msg.data)
        
    def _4DD(msg: can.Message): #1245
        _1245 = Message(msg.timestamp,'1245',msg.data)
        
    def _6E1(msg: can.Message): #1761
        _1761 = Message(msg.timestamp,'1761',msg.data)
        
    def _6E2(msg: can.Message): #1762
        _1762 = Message(msg.timestamp,'1762',msg.data)
